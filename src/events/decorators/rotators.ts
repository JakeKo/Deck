...
export function decorateRotateEvents(svg: SVG.Element, slide: SlideRenderer, graphic: IRotatorRenderer): void {
    ...
    svg.node.addEventListener('mousedown', baseEvent => {
        baseEvent.stopPropagation();
        dispatch(makeRotatorMouseEvent(ROTATOR_EVENTS.MOUSEDOWN, slide, graphic.parent.id, baseEvent));
        dispatch(makeSlideMouseEvent(SLIDE_EVENTS.MOUSEDOWN, slide, graphic, baseEvent));
    });

    svg.node.addEventListener('mouseover', baseEvent => {
        baseEvent.stopPropagation();
        dispatch(makeRotatorMouseEvent(ROTATOR_EVENTS.MOUSEOVER, slide, graphic.parent.id, baseEvent));
    ...
    svg.node.addEventListener('mouseleave', baseEvent => {
        baseEvent.stopPropagation();
        dispatch(makeRotatorMouseEvent(ROTATOR_EVENTS.MOUSEOUT, slide, graphic.parent.id, baseEvent));
        dispatch(makeSlideMouseEvent(SLIDE_EVENTS.MOUSEOUT, slide, graphic, baseEvent));
    });

    svg.node.addEventListener('mousemove', baseEvent => {
        baseEvent.stopPropagation();
        dispatch(makeRotatorMouseEvent(ROTATOR_EVENTS.MOUSEMOVE, slide, graphic.parent.id, baseEvent));
